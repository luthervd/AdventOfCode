...
public class Day12 : IPuzzle<Day12Args, Day12Results>
{
    ...
    public Day12Results Run(Day12Args input)
    {
        ...
        {
            Part1 = Paths.AStar(graph, input.PathGrid.Start.Item1, input.PathGrid.End.Item1, Manhattan).Distance,
            Part2 = input.PathGrid.Nodes.Where(x => x.Value == 'a').Select(x => Paths.AStar(graph, x.Key, input.PathGrid.End.Item1, Manhattan).Distance).Min()
        };
    }
